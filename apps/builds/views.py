...
from .models import Build, BuildComponent, BuildLike
...
def build_detail(request, slug):
    ...
    user_liked = (
        ...
        build.likes.filter(user=request.user).exists()
    )
    context = {
        'build':       build,
...
@login_required
def build_toggle_like(request, slug):
    """AJAX — додати або прибрати лайк/обране."""
    if request.method == 'POST':
        build = get_object_or_404(Build, slug=slug)
        like, created = BuildLike.objects.get_or_create(user=request.user, build=build)
        if not created:
            like.delete()
            liked = False
        else:
